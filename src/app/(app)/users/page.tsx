...
import { Edit, Trash2, PlusCircle, Search, FilterX, ShieldCheck, UserCog, UserPlus, CheckCircle } from 'lucide-react';
import {
    AlertDialog,
    AlertDialogAction,
    AlertDialogCancel,
...
export default function UsersPage() {
  ...
  const filteredUsers = useMemo(() => {
    ...
    return allUsers.filter(user => {
        ...
        const matchesAudit = auditFilter === 'all' || user.auditStatus === auditFilter;
        return matchesSearch && matchesRole && matchesAudit;
    });
  }, [allUsers, searchQuery, roleFilter, auditFilter]);

  const handleAuditVerify = (user: UserProfile) => {
    if (!firestore) return;
    updateDocumentNonBlocking(doc(firestore, 'users', user.id), { auditStatus: 'verified' });
    toast({ title: 'Audit Finalized', description: `Identity for ${user.name} has been verified.` });
  };
  ...
  const onUserCreate = async (values: z.infer<typeof userCreationSchema>) => {
    ...
    try {
        ...
        const userData: any = {
            ...
            role: values.role,
            department: values.department || '',
            auditStatus: 'verified', // Admin-created users are auto-verified
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <div className="relative">
                        ...
                        <Input placeholder="Search directory..." className="pl-10 h-11 rounded-xl bg-white border-slate-200 shadow-sm w-[200px] text-sm font-medium text-slate-900" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                    </div>
                    <Select value={roleFilter} onValueChange={setRoleFilter}>
                        <SelectTrigger className="w-[140px] h-11 rounded-xl border-slate-200 bg-white text-[10px] font-black uppercase tracking-widest"><SelectValue placeholder="Role" /></SelectTrigger>
                        <SelectContent className="rounded-xl">
                     ...
                     <Select value={auditFilter} onValueChange={setAuditFilter}>
                        <SelectTrigger className="w-[140px] h-11 rounded-xl border-slate-200 bg-white text-[10px] font-black uppercase tracking-widest"><SelectValue placeholder="Status" /></SelectTrigger>
                        <SelectContent className="rounded-xl">
                    ...
                    {(searchQuery || roleFilter !== 'all' || auditFilter !== 'all') && (
                        <Button variant="ghost" size="icon" onClick={() => { setSearchQuery(''); setRoleFilter('all'); setAuditFilter('all'); }} className="h-11 w-11 rounded-xl"><FilterX className="h-4 w-4" /></Button>
                    )}
                </div>
            </div>
        </CardHeader>
          ...
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow>
                ...
                <TableHead className="uppercase text-[10px] font-black tracking-widest">Audit Status</TableHead>
                <TableHead className="text-right pr-8 uppercase text-[10px] font-black tracking-widest">Action Terminal</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {isLoading ? (
                [...Array(5)].map((_, i) => <TableRow key={i}><TableCell colSpan={4} className="pl-8 pr-8"><Skeleton className="h-12 w-full rounded-xl" /></TableCell></TableRow>)
                  ...
                  <TableRow key={user.id} className="hover:bg-indigo-50/30 group transition-colors">
                    <TableCell className="pl-8">
                      <div className="flex items-center gap-4 py-1">
                        ...
                        <div>
                            <div className="font-black text-slate-800 uppercase tracking-tight leading-tight">{user.name}</div>
                            <div className="text-[9px] font-bold text-slate-400 mt-0.5">{user.email}</div>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                      <Badge variant="secondary" className={cn("rounded-lg font-black uppercase text-[9px] tracking-[0.1em] px-2.5 py-1", user.role.includes('admin') ? 'bg-indigo-500 text-white' : 'bg-slate-100 text-slate-500')}>
                        {user.role.replace('-', ' ')}
                      </Badge>
                    </TableCell>
                    <TableCell>
                        {user.auditStatus === 'pending' ? (
                            <Badge variant="destructive" className="animate-pulse rounded-lg text-[9px] font-black uppercase tracking-widest">Pending Audit</Badge>
                        ) : (
                            <Badge className="bg-green-500/10 text-green-600 border-green-500/20 rounded-lg text-[9px] font-black uppercase tracking-widest">Verified</Badge>
                        )}
                    </TableCell>
                    <TableCell className="text-right pr-8">
                        <div className="flex justify-end gap-2">
                            {user.auditStatus === 'pending' && (
                                <Button size="sm" onClick={() => handleAuditVerify(user)} className="rounded-xl h-9 bg-green-600 hover:bg-green-700 font-black uppercase text-[9px] tracking-widest">
                                    <CheckCircle className="mr-1 h-3.5 w-3.5" /> Verify
                                </Button>
                            )}
                            <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl hover:bg-white" onClick={() => handleEditClick(user)} disabled={user.id === authUser?.uid}>
                ...
                <TableRow><TableCell colSpan={4} className="h-40 text-center font-bold text-muted-foreground uppercase tracking-tighter text-sm opacity-40">No matching identities found</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </CardContent>
        ...
        <AlertDialogContent className="rounded-3xl">
            <AlertDialogHeader>
                ...
                <AlertDialogDescription className="leading-relaxed">
                    This will remove the identity document for <span className="font-black text-slate-900">{deletingUser?.name}</span> from the active directory.
                </AlertDialogDescription>
            </AlertDialogHeader>
